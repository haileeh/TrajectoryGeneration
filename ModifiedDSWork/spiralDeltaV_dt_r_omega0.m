...
for omega_iter = 1:c
    for r0_iter = 1:length(R0)
        for rf_iter = 1:length(RF)
            ...
            %keyboard
            v_SVC_INT = cross(omega_TAR_INT,r_SVC_INT);
            ...
            for i = 1:num_iter
                ...
                % THIS IS THE SYNCHRONOUS REQUIREMENT
                ...
                % Assuming that TAR and UDP are coaligned. This rotates
                % vector into the TAR frame
                % without this req, need to actively command r_SVC_INT
                % instead; perhaps use keepout zone?
